...
get()	값 꺼냄 (값이 없으면 예외 발생) -> 있든 없든 강제로 꺼낸다
...
public class OptionalTest {
    ...
    public void t1() {
        ...
        // optional -> 선물박스
        Optional<String> opt = Optional.ofNullable(name); // Optional.of(value)	절대 null이 아니어야 함

        if (opt.isPresent()) {  // 선물박스에 값이 존재한다면
    ...
    public void t2() {
        ...
        String result = opt.orElseThrow(() -> new RuntimeException("값이 없어요")); // throw 내부에 있어서 생략가능함
    ...
    public void t3() {
        ...
        String result = opt.orElseGet(() -> "metacoding");  // 디폴트값을 줄 수 있음
        System.out.println(result);
    }
